...
class CUtilMgrDescActions : public FPStaticInterface
{
  protected:
	...
	BEGIN_FUNCTION_MAP
	  FN_3(fnRunModule, TYPE_INT, thePlugPanel.ExecFP_Call, TYPE_STRING, TYPE_STRING, TYPE_STRING_TAB)
...
CUtilMgrDescActions CUtilMgrDescActions::theMaxMgrDescActions(
  ...
  fnRunModule, _T("RunModule"), IDS_RUNMODULE, TYPE_INT, 0, 3,
   _T("prm1"), IDS_PRM1, TYPE_STRING,     f_inOut, FPP_IN_PARAM,
...
int CMaxMgrPanel::_RegisterButtons()
{
  ...
  DWORD myfont   = SendMessage(GetDlgItem(hWnd, IDC_VERSION), WM_GETFONT, 0, 0);
...
static BOOL CALLBACK g_CMaxMgrPanelProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  switch(msg) 
  {
    case WM_INITDIALOG:
...
int CMaxMgrPanel::ExecFP_Call(char *module, char *_cmd, Tab<char*> *_args)
{
  _OpenDLLs();
  ...
  if(!found) {
    char buf[512];
    sprintf(buf, "Can't find module \"%s\"", module);
