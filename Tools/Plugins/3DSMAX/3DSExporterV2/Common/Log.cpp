...
#define LOG_SHARED_FILE_NAME	"!3DSLogWnd01!"
#define LOG_WINDOWS_NAME	"3DSMax Log Window 0.1"
...
CLog *CLog::CreateWnd()
{
  ...
  wnd->m_SharedHandle = CreateFileMapping((HANDLE)0xFFFFFFFF, NULL, PAGE_READWRITE, 0, 8192, LOG_SHARED_FILE_NAME);
  wnd->m_SharedData   = (char *)MapViewOfFile(wnd->m_SharedHandle, FILE_MAP_READ, 0, 0, 0);
...
CRect CLog::btn1_Rect()
{
  ...
  rect.top     = rect.bottom - BUTTON_WIDTH;
  rect.top    += 1;
  rect.left   += 1;
  rect.right  -= 1;
  rect.bottom -= 1;
...
BOOL CLog::OnCommand(WPARAM wParam, LPARAM lParam) 
{
  switch(wParam) 
  {
    ...
    case CLOG_SHOW_MSG:
    {
      if(m_SharedData) Log(m_SharedData);
      break;
    }
  }	
