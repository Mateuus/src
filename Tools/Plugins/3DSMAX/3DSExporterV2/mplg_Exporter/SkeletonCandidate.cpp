...
BOOL CSkeleton::AddNode(INode *pNode, int parentId)
{
  ...
  if(gMaxHelper.IsBone(pNode) || gMaxHelper.IsDummy(pNode))
  {
    CBone *pBone;
    ...
    for(int i=0; i<strlen(pNode->GetName()); i++)
      dwSkeletonID += (pNode->GetName()[i]) * m_vBones.size();
...
CBone * CSkeleton::CreateBone(INode* pNode, int parentId) 
{
  ...
  // create the bone 
  pBone->Create(m_vBones.size(), parentId, pNode);
...
BOOL CSkeleton::CreateFromInterface()
{
  ...
  dwSkeletonID = (dwSkeletonID & 0x00FFFFFF) | (m_vBones.size() << 24);
