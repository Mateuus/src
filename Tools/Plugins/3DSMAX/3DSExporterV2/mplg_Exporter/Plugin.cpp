...
void U_DumpParamBlock(IParamBlock2 *pbl)
{
  ...
  for(int i=0; i<pbl->NumParams(); i++) {
    ...
    switch(pdef.type) {
      ...
      case TYPE_STRING:		{TCHAR* v;	pbl->GetValue(pdef.ID, 0, v, FOREVER);	sprintf(value, "%s", v);		break;}
      case TYPE_FILENAME:	{TCHAR* v;	pbl->GetValue(pdef.ID, 0, v, FOREVER);	sprintf(value, "%s", v);		break;}
      case TYPE_BITMAP: {
        PBBitmap* v;	
...
int handle_FP_Call(expParams *exp_prm)
{
  ...
  for(int i=2; i<prm.Count(); i += 2) {
    if(NULL == _stricmp(prm[i], "LODExport")) {
      theExporter.m_bExportLOD = *prm[i+1] == '1';
    } else
    if(NULL == _stricmp(prm[i], "LODPercents")) {
      strcpy(theExporter.m_LodPercents, prm[i+1]);
    } else
    if(NULL == _stricmp(prm[i], "LODDistances")) {
      strcpy(theExporter.m_LodDistances, prm[i+1]);
    } else
    if(NULL == _stricmp(prm[i], "LODMinFaces")) {
      theExporter.m_LodMinFaces = atoi(prm[i+1]);
    } else
    {
      char buf[512];
  ...
  try 
  {
    ...
    if(_stricmp(exp_prm->FP_cmd, "ExportBoneAnimation") == NULL) {
...
__declspec(dllexport) int expExecFunc(int id, expParams* prm, int* modeless)
{
  ...
  theExporter.m_bShowConfig = 0; //@ disabled vShowConfig.var;
