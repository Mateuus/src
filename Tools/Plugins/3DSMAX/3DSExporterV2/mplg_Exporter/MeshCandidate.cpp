...
D_LOG(1);
  ...
  int MeshMaterialCount = m_pMesh->m_vectorMtl.size();
...
{
	if(fabs(v->m_position.x - 0.305200/200) < 0.01f && 
	   fabs(v->m_position.z - 442.356598/200) < 0.01f && 
	   fabs(v->m_position.y - -928.686646/200) < 0.01f) {
		return true;
	}
	return false;
}
...
void CMesh::SubMat_RemoveDups(SubMaterial* sbm)
{
  ...
  /*
  if(iNumE>10000) {
  FILE* f = fopen("z:\\shit.txt", "wt");
  for(int i=0; i<iNumE; i++) {
    fprintf(f, "%d: %f %f %f %f %f %f %f %f %f\n", 
      i, 
      vsort[i]->m_position.x, vsort[i]->m_position.y, vsort[i]->m_position.z,
      vsort[i]->m_normal.x, vsort[i]->m_normal.y, vsort[i]->m_normal.z,
      vsort[i]->m_tangent.x, vsort[i]->m_tangent.y, vsort[i]->m_tangent.z);
  }
  fclose(f);
  }
  */
...
void CMesh::Finalize_SubMat(SubMaterial* sbm)
{
  ...
  if(g_SCO_ExportTangents) SubMat_CalcTangentSpace(sbm);
