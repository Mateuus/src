...
int CMaxMesh::GetFaceMaterialId(int faceId)
{
  ...
  int materialCount = m_vectorMtl.size();
  if(!materialCount)
    return 0;
...
CVertex *CMaxMesh::GetVertex(int faceId, int faceVertexId)
{
  ...
  // get vertex id
  int vertexId;
  vertexId = m_pIMesh->faces[faceId].v[m_iFaceOrder[faceVertexId]];

 // position / normal
  pVertex->m_position = m_pIMesh->getVert(vertexId) * m_ObjTM;
  pVertex->m_normal   = GetVertexNormal(faceId, vertexId) * Inverse(Transpose(m_ObjTM));
