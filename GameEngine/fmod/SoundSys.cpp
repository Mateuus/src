...
int CSoundSystem::Close()
{
	...
	if (FMODEventSystem)
	{
		...
		FMODEventSystem->unloadFSB("Ambient_Streamed.fsb", 0);
...
int CSoundSystem::LoadSoundEffects(const char *basedir, const char *fname)
{
	...
	rv = FMODSystem->createSound("Data/Sounds/Ambient_Streamed.fsb", FMOD_CREATECOMPRESSEDSAMPLE, 0, &m_SoundBank_Streaming); SND_ERR_CHK(rv);
	rv = FMODEventSystem->preloadFSB("Ambient_Streamed.fsb", 0, m_SoundBank_Streaming); SND_ERR_CHK(rv);
...
//	rv = FMODSystem->createSound("Data/Sounds/WarZ_Music.fsb", FMOD_CREATECOMPRESSEDSAMPLE, 0, &m_WarZ_MusicBank); SND_ERR_CHK(rv);
//	rv = FMODEventSystem->preloadFSB("WarZ_Music.fsb", 0, m_WarZ_MusicBank); SND_ERR_CHK(rv);
#endif
...
void CSoundSystem::Update(const r3dPoint3D &pos, const r3dPoint3D &dir, const r3dPoint3D& up)
{
	...
	result = FMODEventSystem->getCategory("voicecommands", &eventCat); SND_ERR_CHK(result);
