...
bool r3dFSBuilder::ExistInBaseArchive(const file_s& file, int curFile, int totFiles)
{
  ...
  // need to compare CRC
  r3dOutToLog("comparing %s [%04d/%04d]\r", file.name, curFile+1, totFiles); 
...
bool r3dFSBuilder::CreateArchive(const char* name)
{
  ...
  // rename bin files to gz files
  {
	  for(int i=0; i<r3dFileSystem::MAX_VOLUMES; ++i)
	  {
		  char tempStr[512];
		  sprintf(tempStr, "%s_%02d.bin", vname, i);
		  if(r3d_access(tempStr, 0)!=0)
			  continue;
		  char newStr[512];
		  sprintf(newStr, "%s_%02d.gz", vname, i);
		  rename(tempStr, newStr);
	  }
  }
